...
const Dropdown = () => {
  ...
  const dropdownItems = [
    ...
    { name: "Appetizers", path: "/appetizer" },
    { name: "Calzone", path: "/calzone" },
    { name: "Chicken Pcs and Wings", path: "/chickencorner" },
    { name: "Special Fries", path: "/specialfries" },
        ...
        {dropdownOpen && (
          <div className="absolute right-0 mt-2 w-72 bg-black text-white font-semibold rounded-lg shadow-xl overflow-hidden animate-slide-in transition-transform duration-300 ease-in-out transform origin-top-right">
            {dropdownItems.map((item, index) => (
              <Link key={index} href={item.path} duration={500} >
                <div
